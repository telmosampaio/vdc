...
Configuration CreateDomainController {
    ...
    Import-DscResource -ModuleName xStorage, xActiveDirectory, xNetworking
    ...
    Node localhost
    {
        ...
        xADDomainController SecondaryDC 
        {
            ...
            SiteName = $SiteName
            DatabasePath = "$("$DriveLetter"):\Adds\NTDS"
            LogPath = "$("$DriveLetter"):\Adds\NTDS"
            SysvolPath = "$("$DriveLetter"):\Adds\SYSVOL"
            DependsOn = "[xWaitForADDomain]WaitForPrimaryDC","[WindowsFeature]ADDSInstall"
        }
